...
class NoiseScheduler():
    ...
    def ddim_step(self, model_output, timestep, prev_timestep, sample):
        ...
        # 取得當前步與「跳步後」前一步的 alpha 累乘值
        alpha_prod_t = self.alphas_cumprod[t]
        
        # 如果 prev_t 小於 0 (代表已經走到最後一步了)，則 alpha 設為 1.0 (完全乾淨)
        if prev_t >= 0:
            alpha_prod_t_prev = self.alphas_cumprod[prev_t]
        else:
            alpha_prod_t_prev = torch.tensor(1.0, device=sample.device)

        # 1. 根據模型輸出的雜訊，一口氣預測最初始的乾淨狀態 x_0
        beta_prod_t = 1 - alpha_prod_t
        pred_original_sample = (sample - beta_prod_t ** 0.5 * model_output) / alpha_prod_t ** 0.5

        # 2. 計算指向 x_{t-1} 的方向 (這裡我們捨棄了 DDPM 的隨機變異數，讓 eta=0)
        dir_xt = (1 - alpha_prod_t_prev) ** 0.5 * model_output

        # 3. 算出前一步的狀態
        pred_prev_sample = alpha_prod_t_prev ** 0.5 * pred_original_sample + dir_xt
